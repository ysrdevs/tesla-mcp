#!/usr/bin/env python3
"""
Tesla Fleet API MCP Server (API Key Auth)

Same as tesla_mcp.py but uses StaticTokenVerifier API key auth
instead of OAuth. For clients like Interaction.co's Poke that only support API keys.

Set MCP_API_KEY in .env or let it auto-generate to .api_key
Default port: 8753
"""
...
TESLA_PROXY_URL = os.getenv("TESLA_PROXY_URL", "")
...
logger = logging.getLogger("tesla-mcp-apikey")
...
def get_api_key() -> str:
    ...
    if not api_key:
        if API_KEY_FILE.exists():
            ...
            print(f"[tesla-mcp-apikey] Generated new API key: {API_KEY_FILE}", file=sys.stderr)
    return api_key
...
if __name__ == "__main__":
    ...
    port = int(os.getenv("TESLA_MCP_PORT_APIKEY", "8753"))
